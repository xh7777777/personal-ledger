  ...
  <section class="panel">
    ...
    <div v-else class="recent-list">
      <div v-for="transaction in transactions" :key="transaction.id" class="recent-item">
        <div>
          <div class="recent-title" :title="`${transaction.category}${transaction.target ? ` · ${transaction.target}` : ''}`">
            {{ transaction.category }}<span v-if="transaction.target"> · {{ transaction.target }}</span>
          </div>
